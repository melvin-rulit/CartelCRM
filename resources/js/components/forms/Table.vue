<template>
  <div class="container">
    ...
    <div class="table-footer">
      <button @click="goToPreviousPage" :disabled="currentPage === 1"><Icon icon="ic:round-arrow-back" class="ico" />Назад</button>
      <div class="pagination-numbers">
        <button v-for="page in totalPagesArray" :key="page" :class="{ active: page === currentPage }" @click="goToPage(page)">
          {{ page }}
        </button>
      </div>
      <button @click="goToNextPage" :disabled="currentPage === totalPages">Вперед<Icon icon="ic:round-arrow-forward" class="ico" /></button>
    </div>
  </div>
</template>
